...
export default function Fantasy(props) {
  ...
  const {data: Fantasy, isLoading,isFetched} = useQuery(["Fantasy"], ()=>{
    return axios.get(`${props.api_base_url}/discover/movie?api_key=${props.api_key}&language=en-US&&with_genres=14&page=${random_keySciFi}`).then((res) => res.data.results);
  });
  ...
  const loadTrailer_Fantasy = async () => {
    const res = await axios.get(`${props.api_base_url}/movie/${MOVIE_ID_Fantasy}/videos?api_key=${props.api_key}`);
